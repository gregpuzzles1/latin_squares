  ...
  void _handleOrderChange(String value) {
    ...
    setState(() {
      if (result.isValid && result.value != null) {
        _errorText = null;
        _currentSquare = LatinSquareGenerator.generate(startingOrder: result.value!);
      } else {
                            ...
                            children: [
                              ...
                              InkWell(
                                onTap: () => _launchExternal(
                                  Uri.parse('https://github.com/gregpuzzles1/latin_squares'),
                                ),
                                child: Text(
                                  'Repository',
                                  style: Theme.of(context).textTheme.bodySmall?.copyWith(
                              ...
                              InkWell(
                                onTap: () => _launchExternal(
                                  Uri.parse('https://github.com/gregpuzzles1/latin_squares/issues/new'),
                                ),
                                child: Text(
                                  'Open an Issue',
                                  style: Theme.of(context).textTheme.bodySmall?.copyWith(
...
class _ThemeToggleMenuEntry extends PopupMenuEntry<void> {
  final bool isDarkMode;
  final ValueChanged<bool> onToggle;

  const _ThemeToggleMenuEntry({
    required this.isDarkMode,
  ...
  @override
  State<_ThemeToggleMenuEntry> createState() => _ThemeToggleMenuEntryState();
}

class _ThemeToggleMenuEntryState extends State<_ThemeToggleMenuEntry> {
  @override
  Widget build(BuildContext context) {
    return Padding(
